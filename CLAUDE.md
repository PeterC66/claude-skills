...
**A red `main` here does not stay here.** `actions/checkout` fetches this repository with **no `ref:`** twice in `community-bus-maps/.github/workflows/test.yml` (the hygiene checker, then the document checkers), so each of those runs whatever `tools/` is on this `main` **at the moment it runs** — a broken `tools/` reddens the portal under a commit that never touched this repository. `buses-data/.github/workflows/gates.yml` no longer floats: it checks this repository out at the engine commit pinned in its `engine.lock.json` (OA-398), so a change here — the engine, or a checker such as `stamp-docs/scripts/check_committed_stamps.py` that runs from that checkout — reaches that job only when the pin is bumped with the rebuilt sheets. It is the same mechanism `buses-data`'s CLAUDE.md describes from the other end, and it is why this `main` being green is the portal's business too.
...
- **There is nothing to stamp here.** `claude-skills` is not a root in `stamp-docs/stamp-policy.json` (the roots are `buses`, `portal` and `ops`), so its documents carry no docstamp and `docstamp.py --checkout .` refuses a worktree of it. In the two repositories that are roots, the pre-commit hook stamps a worktree's commits by itself.
- **Removing it when the PR merges is no longer yours to remember** (2026-09-23). `bus-work/assets/worktree_sweep.mjs` runs weekly from Windows Task Scheduler and removes every worktree, in all three repositories, whose work is on `origin/main` or in a merged pull request and which is clean and a day idle; a merged one it refuses — dirty, locked, holding a junction — becomes a worklist row. If you remove one by hand, use `git worktree remove` rather than deleting the directory, so the administrative entry goes with it. `git -C "C:/u3a St Ives/.claude/skills" worktree list` is the read that says what is actually there.
