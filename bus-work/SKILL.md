...
**`worklist.mjs --conditions --json`.** The loop reads the per-resource verdicts out of the JSON and stops unless every resource its intended work touches says `safe`. It never reads the exit code, because that command always exits 0. **An unattended run collapses `CHECK FIRST` into stop** — the three bands were written for somebody who can go and read whose uncommitted files those are, and there is nobody. So a verdict that becomes vaguer, or a resource key that is renamed, silently changes what an unattended run will refuse to do.
