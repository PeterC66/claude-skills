...
**The dry run's verdict decides which write to run, and the common one needs `--force`.** A map whose sheets already gate PASS under the live template, with only its `engine` stamp old, answers **STAMP-STALE**, and the tool will not rebuild it without `--apply --force`. It prints that exact line itself; the row carries it as its third step. It is safe there because the tools reach STAMP-STALE only after the STALE-INPUTS and UNRENDERED guards have passed, and only when every sheet is byte-identical under the live template, so the rebuild moves the build stamp and nothing else — Beaconsfield Simpson Centre v1.38 → v1.39 on 2026-09-25 (buses-data `1f8a7382`) was exactly that. **`--force` is one flag with four meanings in these tools**: rebuild a stamp-stale map, finish an UNRENDERED S4, roll old geometry forward past STALE-INPUTS, and publish past a lost label. Only the first belongs to this row. A STALE-INPUTS verdict is a data change and goes through the map's build order instead, and a lost label is reviewed, never forced. A stamp-only rebuild changes nothing a customer sees, so it is committed and pushed but not delivered to the portal, where delivery would email the customer about an unchanged sheet.
