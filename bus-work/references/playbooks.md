...
2. **Stage it**, in `PORTAL` (`C:\Claude\community-bus-maps`):
   ...
   - **A town in the month's ink review — use the gated step** (buses-data OA-428). It runs from any folder and the path is real; `<scan date>` is the report the row is joined to, `<Town>` is the town as `Areas/` spells it, `<slug>` is the `slug` the refresh row carries, and `<who>` is your own session name:
     ...
     Leave `--apply` off and it prints the one `npm run deliver` command it would run, and runs nothing. It refuses before anything leaves the laptop if the town is not under `deliver` in `ink_review.mjs --deliverable`, if the newest S5 render is not the build the review compared, or if that build was already staged (a second run would be a second email). After the deliver command it records `staged` on the map in `_gtfs/ink-review_<scan date>.json` and reads the portal back for the update waiting on its customer, which needs `BUSMAPS_URL` and `BUSMAPS_TOKEN` from the portal's `.env` — hence `--env-file` in the command above: without it the command stages and emails the customer, and only then fails to read the portal back. A scheduled tick may run exactly this, for a town under `deliver` and no other (buses-data OA-428). **A non-zero exit is a hold, never a retry**: past the deliver command's step 4 the customer may already have been emailed. It stops at *staged*; publication is the customer's Accept.

   - **Against the live site, by hand** (item 4, 2026-08-10): `npm run deliver -- --map <slug> --kind area|place --src "<fresh S5-render dir>" --note "BODS <date> refresh"`. One laptop command — scp's the render to the VPS, verifies it byte-identical *before touching the live service*, stops the portal, runs `propose-update.mjs` inside a throwaway container, restarts, health-checks. Needs `DEPLOY_HOST`/`DEPLOY_SSH_KEY`/`DEPLOY_APP_DIR` in `.env`. `worklist.mjs` prints exactly this form when it's reading the live worklist (`--url`/`BUSMAPS_URL` set).
...
- No `--yes` prints the full list and waits for you to type "yes"; add it once you trust the plan without re-reading it printed back. **The prompt needs a terminal**: since 2026-09-05 (OA-248) a run without `--yes` from a non-interactive shell is refused with exit 2 *before* anything is minted, because until then it printed the prompt, waited on input that could not come, and exited 0 leaving a live admin session on the host.
- `--dry-run` shows the plan with no `--cookie`/`--mint` needed and makes no calls at all — use it to sanity-check `--only` before spending a real session on it. **Add `--cookie` to a dry run and it reads the pending list** over one GET and prints it, still changing nothing — the way to see what a run would do without minting.
- Whatever exit a `--mint` run takes — the prompt declined, an error thrown, the happy path — the minted session is revoked in a `finally` and read back as gone (OA-248); a revoke it cannot confirm exits non-zero, and that is the moment to check the host's `session` table by hand.
...
**`landmark-owed-<slug>` — the portal holds an answer the town's source lacks.** From `SK` (the engine's own folder), with `--town` the folder name under `Areas/` and nothing else required:

```bash
node "%SK%\poi_tiers_sync.js" --town "High Wycombe"
```
...
```bash
node "%SK%\rollout.js" --town "High Wycombe" --force
```
...
**The dry run's verdict decides which write to run, and the common one is `--apply --rebuild-stale`.** A map whose sheets already gate PASS under the live template, with only its `engine` stamp old, answers **STAMP-STALE**, and the tool rebuilds it only with `--apply --rebuild-stale`. It prints that exact line itself; the row carries it as its third step. It is safe because the tools reach STAMP-STALE only after the STALE-INPUTS and UNRENDERED guards have passed, and only when every sheet is byte-identical under the live template, so the rebuild moves the build stamp and nothing else — Beaconsfield Simpson Centre v1.38 → v1.39 on 2026-09-25 (buses-data `1f8a7382`) was exactly that. Until 2026-09-26 this step was `--apply --force`, and **`--force` is one flag with four meanings in these tools**: rebuild a stamp-stale map, finish an UNRENDERED S4, roll old geometry forward past STALE-INPUTS, and publish past a lost label or a blocking warning. Auto mode rightly refused it to a loop tick, so every row waited on a person (buses-data OA-473). `--rebuild-stale` is the first meaning and none of the others: it never sets `--force`, a map in any state but STAMP-STALE is refused as NOT-STAMP-STALE or left UP-TO-DATE, and the two flags together are a usage error. `make-bus-leaflet/tools/prove-red-rollout-stamp.js` proves each of those refusals still bites with the flag set. A STALE-INPUTS verdict is a data change and goes through the map's build order instead, and a lost label is reviewed, never forced. A stamp-only rebuild changes nothing a customer sees, so it is committed and pushed but not delivered to the portal, where delivery would email the customer about an unchanged sheet.

**Find a map's latest S4 run from its `manifest.json`, `stages.S4.latest` — never `ls S4-generate/*/ | tail -1`**, which sorts by name and so puts `v3.12` before `v3.9`: on 2026-09-25 that picked Ramsey's `v3.9` run instead of `v3.12`.
