...
2. **Stage it**, in `PORTAL` (`C:\Claude\community-bus-maps`):
   ...
   - **A town in the month's ink review — use the gated step** (buses-data OA-428). It runs from any folder and the path is real; `<scan date>` is the report the row is joined to, `<Town>` is the town as `Areas/` spells it, `<slug>` is the `slug` the refresh row carries, and `<who>` is your own session name:
     ...
     ```bash
     node "C:/u3a St Ives/.claude/skills/bus-work/assets/stage_refresh.mjs" --scan <scan date> --town "<Town>" --map <slug> --apply --by <who>
     ```

     Leave `--apply` off and it prints the one `npm run deliver` command it would run, and runs nothing. It refuses before anything leaves the laptop if the town is not under `deliver` in `ink_review.mjs --deliverable`, if the newest S5 render is not the build the review compared, or if that build was already staged (a second run would be a second email). After the deliver command it records `staged` on the map in `_gtfs/ink-review_<scan date>.json` and reads the portal back for the update waiting on its customer, which needs `BUSMAPS_URL` and `BUSMAPS_TOKEN` from the portal's `.env`. **A non-zero exit is a hold, never a retry**: past the deliver command's step 4 the customer may already have been emailed. It stops at *staged*; publication is the customer's Accept.
