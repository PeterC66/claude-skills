...
// ---- portal vendored FIXTURES (gate-fixtures/ in the portal) ---------------
// buses-data OA-419. The block above asks whether the portal's copy of the ENGINE
// has fallen behind this skill tree. It has the portal checkout open and it does
// not ask the other half of the same join: whether the portal's copy of
// buses-data's GATE FIXTURES has fallen behind buses-data.
//
...
function portalFixtureVendoring() {
  if (!exists(PORTAL)) return null;
  const source = portalSource();
...
// OA-419. Deliberately NOT in `bad` below: a fixture the portal has not re-vendored
// is a chore, and the worklist carries it as `portal-fixtures-behind`.
const fixtureVendoring = portalFixtureVendoring();
...
async function main() {
  ...
  // OA-419. Printed whether or not anything is behind, for the reason the section
  // above prints its source line: a verdict nobody ever sees green is one nobody
  // learns to read. A chore, so it never touches the exit code.
  if (fixtureVendoring) {
    console.log('\n=== Portal vendored fixtures (buses-data -> portal gate-fixtures/, CRLF-safe) ===');
    const fSrc = driftSourceLine(fixtureVendoring.source);
    if (fSrc) console.log('  ' + fSrc);
    if (fixtureVendoring.status === 'NO-SOURCE') {
      console.log('  NOT ASKED  ' + fixtureVendoring.why);
    } else if (fixtureVendoring.status === 'in step') {
      console.log('  in step  every fixture file the portal vendors is byte-identical to buses-data\'s ('
        + fixtureVendoring.kinds.map((k) => k + '/_portal-fixture').join(', ') + ')');
    } else {
      for (const b of fixtureVendoring.behind) console.log('  ' + b.state.padEnd(9) + b.file);
      console.log('  BEHIND   ' + fixtureVendoring.behind.length + ' file(s). A CHORE, not a fault -- the artwork is'
        ...
        + '\n           Remedy, from ' + PORTAL + ':   npm run fixtures:vendor -- --apply');
    }
  }
