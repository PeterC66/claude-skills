 ...
 *
 * WHY A FOURTH QUESTION AT ALL. S-5 above asks which way a route leaves town,
 * and it can only ask it of a route with two or more out-of-town buffer stops:
 * 26 of 95 displayed routes across the eight towns, none at all on March or
 * High Wycombe. Two instruments for widening it were built and measured on
 * 2026-08-29 and both manufactured findings on correct sheets, for one shared
 * reason — both sides of the comparison come from the same chain, so neither can
 * disagree with the other about a DRAWING error. What CAN be wrong is
 * derive_intown's drawn stop set, and this check asks about that instead: not
 * where the road goes, but whether the window we cut out of it has a hole.
 *
 ...
 * that says otherwise. */
const winChecked = [], winUnavailable = [], winSkipped = [];
let winAnyDirGappy = 0;
...
const WINDOW_COMPLETENESS_FLOOR = 0.5;
{
  ...
  for (const r of displayed) {
    ...
    if (!corePrefix) { winSkipped.push({ route: r, reason: 'no-intown-prefix' }); continue; }
    if (CIRCULAR.has(r)) { winSkipped.push({ route: r, reason: 'circular' }); continue; }
    const seq = intownByNorm(r);
    const fe = fullEntry(r);
    if (!fe || !seq || seq.length < 2) {
      winSkipped.push({ route: r, reason: !fe ? 'no-full-chain' : 'fewer-than-two-drawn-stops' });
      continue;
    }
    const drawn = new Set(seq);
    const per = [];
    ...
    if (!per.length) {
      winUnavailable.push({ route: r, reason: 'fewer-than-two-in-town-calls-drawn-in-any-direction' });
      continue;
    }
    winChecked.push(r);
    const gappy = per.filter(p => p.gaps > 0);
    if (gappy.length) winAnyDirGappy++;
...
const out = {
  ...
  summary: {
    ...
    /* How much of S-5b ran (OA-048, 2026-09-15). Same contract as the two above,
     * for the same reason: a check that can decline has to say how often it did.
     * `anyDirectionGappy` is the count BEFORE the completeness floor — two across
     * the estate today, of which one speaks — recorded as a number so the
     * narrowing is visible to a reader, and so the floor can be re-measured from
     * the reports rather than from the comment that argues for it. */
    windowCoverage: {
      checked: winChecked.length,
      unavailable: winUnavailable.length,
      unavailableBy: winUnavailable.reduce((a, u) => (a[u.reason] = (a[u.reason] || 0) + 1, a), {}),
      skipped: winSkipped.length,
      skippedBy: winSkipped.reduce((a, s) => (a[s.reason] = (a[s.reason] || 0) + 1, a), {}),
      displayed: displayed.size,
      pct: displayed.size ? Math.round(100 * winChecked.length / displayed.size) : 0,
      accountsForAll: winChecked.length + winUnavailable.length + winSkipped.length === displayed.size,
      anyDirectionGappy: winAnyDirGappy,
    },
  },
  findings,
};
...
// it is the check that reaches the routes S-5 cannot, and a reader comparing the
// two lines is reading the coverage argument OA-048 was opened about.
...
console.log(`        drawn-window check ran on ${wc.checked}/${wc.displayed} displayed routes (${wc.pct}%)`
  ...
  + (wc.anyDirectionGappy ? `; ${wc.anyDirectionGappy} gappy in SOME direction but not all, which this check does not raise on` : ''));
