...
**Where the band is read after S2 (since 2026-09-05, buses-data OA-088).** `complexity.json` rides with every AREA delivery — S4 pulls S2's outputs, S5 copies them on, and the portal's importer keeps every `*.json` — and for five weeks nothing on the far side read it, so Wisbech was published RED with nobody told. The portal's review screen now shows the band beside the build verdict and writes it into the publish evidence (`readComplexity()` in `src/maps/store.js`, portal #237), and the bus-work worklist prints it beside an AREA map request — or says UNSCORED with the S1→S2 step, which costs no quota — so a RED town is met BEFORE approval spends the customer's slot (`bus-work/assets/complexity_band.mjs`). Two things to know when reading a band there: the band in a pack is the band of the S2 the pack was built FROM, so an older pack can say GREEN on four measures where the newest S2 says RED on P; and it moves only when S2 is re-run, not on a monthly refresh.
