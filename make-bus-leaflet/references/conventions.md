...
**Node 24** — `gates.yml` installs 24 in the `unit` and `docs` jobs, and the `status` job reads the major version off the deployment image so it cannot drift from what actually serves the site. `package.json` here declares `"engines": { "node": ">=24" }`, added by OA-224 Tier 5 in `9ff82bb` with a `//engines` note saying why; this paragraph went on calling that a gap for two days afterwards and was corrected by the 2026-09-03 review (cross-repo F27). The portal's `engines` said `>=22` when that sentence was written and says `>=24` today — it was raised the same day, so the claim of a divergence was wrong within hours of being made and stood for eleven days until the 2026-09-14 review re-measured it (R6 N1). **Read a neighbour's declared version off that repository, never off this page**: three documents in this estate were found asserting a neighbouring repository's state in prose that round, and a sentence about another repository has no checker anywhere, because nothing in `>=22` says which repository it is a claim about.
...
**Read a commit back by CONTENT, not by its stat line.** `git commit -- <paths>` commits the **working-tree** content of those paths, evaluated at COMMIT time — so it is a guarantee about which files land and no guarantee at all about what is in them. Anything that rewrites one of those files between your review and your commit is what gets committed, and the review cannot see a write that happens after it runs. Measured in `buses-data` on 2026-09-09: `6c3eb15` reviewed `git diff HEAD -- <the two paths>` as its own command, read **8 insertions, 4 deletions**, committed those exact paths seventeen seconds later and printed **6 insertions, 4 deletions**; nobody compared the two numbers, and the two table rows the message describes are absent from the tree it made. Nothing went red — hook passed, every checker green on the working tree, exit 0 — and the loss was found five hours later only because a checker was run against a throwaway worktree of `HEAD`. **The rewriter there was the docstamp Stop hook, which does not run in this repository** — `stamp-policy.json` covers `buses-data`, the portal and `community-bus-maps-ops`, and nothing here. So the hazard here is the plainer one: another session Writing the same file while you have it open, which this estate has repeatedly had happen. The rule is about the property of a pathspec commit rather than about any one rewriter, so it holds either way — finish with `git show HEAD:<file>` grepped for the phrase the message promises, and treat a commit whose stat differs from a review taken seconds earlier as a lost update until proven otherwise.
