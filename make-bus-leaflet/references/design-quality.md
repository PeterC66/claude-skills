...
**Choosing a colour for a route that is NOT YET on the sheet — the case the tool refuses.** `pick_route_colour.js` resolves the town to `Areas/<Town>/ci-reference/routes.json`, and `ci-reference` mirrors the latest **committed S4**, so a route being added exits 2 with *"route … is not in <Town>'s palette"* — the one situation in which a hue actually has to be chosen. Until OA-226 gives it a flag, build a scratch tree holding `Areas/<Town>/ci-reference/{routes.json, features_geo.json}` copied from the uncommitted S3 and S2 runs and point `--buses` at it. **Do it rather than choosing by eye.** Route 20's provisional hue on High Wycombe was `#88CCEE`, picked for a defensible reason — it is what the town-centre boarding sheet already drew the route in, so our two artefacts would agree — and the tool scored it at **dE 6.8 against the river**. Its own answer, `#E69F00`, scored 28.6.

**And measure what the tool says its numbers cannot see.** Its closing advice is that a hue can be numerically distant and still run alongside the line it must be told apart from. The concrete form of "runs alongside" is **shared road edges in `routes_paths.json`**: normalise each edge to an unordered node pair, intersect the candidate route's set with every other drawn route's, and report the overlap as a percentage of the candidate's own line. On High Wycombe route 20 that gave 36 at 49%, 850 at 33%, and 34, 33 and 32/32A at 12% each, with seven drawn routes it never touches at all — so the colours to be far from are five, not twenty-two. `#E69F00`'s worst against that weighted set is the same 28.6, which is what made it safe to take rather than merely top of a list.
