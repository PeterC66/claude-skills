...
`routes.json` keys. **They were all opt-in and all defaulted to the pre-2026-08-15 behaviour when this block was written, and most of them no longer do** — G5 promoted twelve of them to engine defaults on 2026-08-17 and later rounds promoted more, so the block below is now the LEGACY behaviour written out in full rather than a list of switches anybody has to set. Each promoted key keeps an escape hatch (`false`, or `"v1"` for `labels.engine`); the Default column in the register below is the live answer, key by key:
...
Every built town carries all of these as of 2026-08-16 (towns at Beaconsfield v1.34, High Wycombe v2.38, Huntingdon v3.32, March v2.32, Ramsey v1.31, St Ives v6.45, St Neots v2.33, Wisbech v1.34, engine `ace07f941f`), **except two, which are town-specific rather than universal**: `panelCorridors` only means anything on a town that declares `internalCorridors` (today just High Wycombe), and `spokeSpread` is carried by **Beaconsfield, Huntingdon, March and Ramsey** — the other four are blocked by legend size, not by the bearing rule. `panelCorridors` also needs `corridorDesc` (and optionally `corridorNote`) beside it. Places carried none of this when that was written, and the sentence has not been true since G5: a default reaches a place exactly as it reaches a town, so the twelve places draw with `printSafe`, `iconInk`, `iconSet` and the rest whether or not anybody adopted them. **Read the whole paragraph above as a record of 2026-08-16 and the register's Default column as the live answer** — measured 2026-09-15 by the design-key census, `…\Buses\Development Docs\design-key-census_2026-09-15.md`, which found 21 of the 36 keys set by no committed map and 19 of those 21 on by default. **The portal re-vendor is DONE (2026-08-16) and did not change them** — re-vendoring copies the engine, and every key here defaults off, so the places move only when the keys are *adopted* on them. That is Phase 8 items 3b and 4, not this one.
...
|---|---|---|
| `footerSafe` | **on** | `false` restores the flat `y=205`. **This row said "off" until 2026-09-15 and `gen_internal.js:366` has read `DESIGN.footerSafe !== false` since the G5 defaults of 2026-08-17** — it is on for every sheet the estate draws, and no map sets it. Ends the map frame just above the footer's backing plate instead of at a flat `y=205`. The plate starts at 195.16 mm and is painted last, so a 9.84 mm strip of every sheet used to be drawn and then covered — 12 of 31 sheets had real route ink in it and 9 had erased *text*. The fit is derived from the frame, so the map refits into the space that is actually visible; expect the whole map to shrink by about 6% vertically. |
...
| `spreadIcons` | **on** | Pushes POI symbols apart until they are `iconMinSep` apart, capped at `spreadMax` mm from the true position. Hand-placed POIs (`overrides` `pos`/`move`) are pinned and never moved. **This row said "off" until 2026-08-30 and the code has read `DESIGN.spreadIcons !== false` all along** — it is ON for every map, and the gate cannot check a Default column. It matters because `Areas/Ramsey/REBUILD-NOTES.md` and the public report behind OA-176 both reason about what this pass can and cannot do, and a reader who checked the register would have concluded it was not even running. What it genuinely cannot do is separate a symbol from a route line, a badge or a label: it runs in the claim phase, before any route is drawn, so the only thing in existence to avoid is another symbol. |
...
| `placeIndex` | **on** | The numbered place index (OA-078). Every label the placer drops is offered a NUMBER at the same point — two glyphs where a name of thirty would not go — and the names are listed under the Key as *Numbered on the map*. Board-wide it identifies 85 of the 263 places the internal-family sheets were drawing anonymously; outside High Wycombe's three town sheets that is 68 of 98. **On unless a map sets `false`**, and the same trap as `spreadIcons` above applies to this Default column: the gate checks that the key has a row, never that the row is true. Set an object to tune it — `{size, pitch, headSize, numWidth, marker, gap, cols, heading}` — and note `cols` overrides the automatic choice, which is the fewest columns that hold the rows, because a second column takes width from every name. What it cannot do is make room: on a saturated sheet the marker is dropped rather than forced, and the count that could not be numbered goes to stderr. |
| `panelScale` | **on** | One type scale and one heading rhythm for the Services/Key panel. `false` reverts. **A G5 default since 2026-08-17 (`gen_internal.js:368`, `!== false`); this row said "off" until 2026-09-15 and no map sets it.** Section below. |
...
| `badgeFit` | **on** | Draws a route badge as a **stadium** instead of a disc when its number is wider than the disc. `false` reverts. **A G5 default since 2026-08-17, read `!== false` by both generators (`gen_internal.js:371`, `gen_external_radial.js:177`); this row said "off" until 2026-09-15 and no map sets it.** Section below. |
| `hubFit` | **on** | External sheets: sizes the hub box from its text instead of from a character count. `false` reverts. **A G5 default since 2026-08-17 (`gen_external_radial.js:333`, `!== false`); this row said "off" until 2026-09-15 and no map sets it.** Section below. |
...
| `exitCaptionsInPanel` | off | Lets a *"to X"* exit caption that has no room on the map take the empty panel column **below** the Services panel, Key and notes, instead of becoming a number in the place index. Only exit captions get the space: every other label keeps the sheet's hard limit at the map's right edge, the panel is blocked down to its real bottom (measured by a silent dry run of `drawServicesPanel`), and map notes, the print-safe margin and the footer plate stay blocked. A caption placed there is claimed as ink before the place index measures its room. Absent, every sheet is byte-identical. Asked for on Soham's internal sheet, whose T5 "to Cambridge" was printed as a **2** beside "to Newmarket" in a corner with a blank column beside it (buses-data OA-416, 2026-09-24). |
| `iconSet` | **on, `"grid"`** | `false` reverts to the twelve legacy drawings, byte-identical. **A G5 default since 2026-08-17 (`gen_internal.js:375` defaults the VALUE to `'grid'`); this row said "off" until 2026-09-15 and no map sets it.** `"grid"` swaps the twelve POI pictograms for the redrawn set: one 24 × 24 grid, one stroke weight, one corner radius, solid, each with a white casing. Pairs with `iconInk`, does not need it. Section below. |
...
| `sheetQr` | **`{mm:14}`** | Draws a QR code of `sheetUrl` at the bottom-right of the footer band. **A default since 2026-08-24**, and the default is what every sheet draws with: **no map sets `sheetQr` at all** — this row said "all 20 maps set exactly `{mm:14}`" until 2026-09-15, and the census of that day found nought of twenty. It only fires where `sheetUrl` is set, so a sheet with no address to point at still draws no code. `false` refuses it; `{mm, level, target}` tunes it — `mm` is the module area's side (14 from the generators, 16 if you pass a bare `true` and let `footer.js` choose), `level` the error-correction level (`L`/`M`/`Q`/`H`, default `M`), `target` a different URL to encode from the one printed. Section below. |
| `howToUse` | **on** | External sheets: a "How to use this map" panel of plain bullets, worded from the town's own data. **On by default since 2026-08-24**, `false` to refuse it. **Three bullets is the DERIVED default since 2026-09-26** (buses-data OA-437): eight of the nine towns that store `bullets` store exactly those three, so a stored list is redundant rather than content (The Shelfords' is not — its hub is worded shorter than its label). `{heading, bullets, at, width, size, headingSize, place}` tunes it. Section below. |
| `printSafe` | **on, `5`** | `5` keeps every drawn thing 5 mm from the trim — footer, placer, panel columns, terminus lozenges — and carries the rest of the 2026-08-16 print check with it. `false` gives today's 8/294/206 geometry, byte-identical. **A G5 default since 2026-08-17, read in all three generators (`gen_internal.js:379`, `gen_external_radial.js:98`, `gen_boarding.js:955`); this row said "off · On all 8 towns; the 5 places do not have it yet" until 2026-09-15, and all three clauses were false — no map sets it, the default reaches places exactly as it reaches towns, and there are twelve places rather than five.** Section below. |
| `iconInk` | **on, `"charcoal"`** | `false` reverts to full colour. **A G5 default since 2026-08-17 (`gen_internal.js:374` defaults the VALUE to `'charcoal'`); this row said "off" until 2026-09-15 and no map sets it.** `"charcoal"` recolours every POI symbol to one neutral, keeping red for the GP cross, so **colour on the sheet means route and nothing else** (G3, Peter, 2026-08-15). Implemented in `icons.js` as a post-pass over the existing drawings, chosen over a redrawn outline set because at 4.2 mm a 0.5 mm outline goes noticeably faint against a ribbon while these solid glyphs hold their weight. Two things it is careful about: a pale fill is a backing plate, not a mark, so it goes white rather than black (the allotments bed); and a symbol that was *already* a neutral grey was drawn light on purpose, so it keeps its tone rather than flattening to charcoal — the industrial estate is context, and a cluster of factories at full charcoal was the heaviest ink on the High Wycombe sheet. |
...
| `laneRibbon` | off | The stronger form of `laneOrientation` — the ribbon-cable order the CORR-001 reader asked for (OA-176 4.21, 2026-09-04): a route keeps its side of travel through every corner, its lane normal comes from a reference segment that runs alongside it rather than the nearest one heading elsewhere, a junction node's 0.1 mm of jitter is read over ±1 mm so a bundle no longer drops a lane for one segment, and the lane vertices are mitred so a bundle holds its gap round a turn. Since 2026-09-05 (OA-176 4.24) the same key also draws a route's own out-and-back ONCE: a return leg beside the out leg is folded onto it before anything reads the polyline, which removes the odd cycle a retrace drawn as two lines forces on the corridor structure (Beaconsfield's five-route town-centre loop is now one line). **Opt-in and inert without `laneOrientation`**; absent ⇒ byte-identical. Adopted by Beaconsfield Simpson Centre only. Section below. |
...
| `legendPlace` | **on** | Lets the legend find its own clear ground instead of sitting in a fixed corner, searching against the same reserved boxes the placer uses. `false` leaves the legend exactly where `legendAt` (or the default 10,40) puts it. **This row said "off · Absent leaves it exactly where it has always been" until 2026-09-15 and both halves were wrong**: it is one of the twelve G5 keys promoted on 2026-08-17 and `gen_external_radial.js:785` has read `!== false` since, so the search runs on every external sheet. Two PLACES set it explicitly — Beaconsfield Simpson Centre and Ely Co-op, `74134db0`, 2026-08-28 — which changes nothing, and no town does. **The block comment at `gen_external_radial.js:763` still says "Absent => the legend stays exactly where legendAt … puts it", which is where this row's wording came from; it is stale in the same way and is left alone deliberately, because a comment edit moves the engine closure hash and makes every town a non-donor for `prove-red-held-back`.** Section below. |
| `labelPrefer` | off | External radial sheets: acts on each branch's `side`, by costing a stop label for sitting away from the perpendicular the spoke steers to (`labeller.js`'s `wPrefer`, a cost and never a rule). **The mechanism was broken and is now fixed; the VALUES are the reason this is off.** `gen_external_radial.js` computed the preference and `labeller.js` read sixteen item properties without it, so 81 of the board's 83 spokes carried a `side` that had done nothing since labels engine v2 became the default. Measured across the eight towns on 2026-08-30: honouring them all at once improved no sheet's numbers and moved one of Ramsey's four "Bury" labels from 31.9 mm to 23.9 mm of another, turning a clean external sheet into a HARD defect — Ramsey states `side:"up"` on all six of its branches, which is what a config looks like when nobody has seen its effect. A town opts in once somebody has looked at its spokes and re-authored them. |
...
**`node "%SK%\pick_route_colour.js" --town "<Town>" --route <key>` does the scoring**: it ranks candidate hues by their **worst** separation against every other colour on that sheet *plus* every linear feature it draws in a chromatic colour, which is the rule that matters — not distance from the colour being replaced. Then render it, because "runs beside" is not in the numbers. **Since OA-226 it also scores each candidate against only the routes it is actually drawn BESIDE, and prints a `sources:` line naming every file it opened** — see the two paragraphs below.

**Choosing a colour for a route that is NOT YET on the sheet — the case the tool used to refuse, and answers since OA-226 (2026-09-03).** `pick_route_colour.js` resolved the town to `Areas/<Town>/ci-reference/routes.json`, and `ci-reference` mirrors the latest **committed S4**, so a route being added exited 2 with *"route … is not in <Town>'s palette"* — the one situation in which a hue actually has to be chosen. Two things changed and the default did not. A route with no colour in the palette is now scored as a **new route** instead of refused, and it says which routes the sheet does draw so a typo in `--route` is still distinguishable from a genuine addition. And the inputs are a flag: **`--stage`** reads the manifest's current S3 (`routes.json`) and S2 (geometry), which is the pair a build in progress has and `ci-reference` by definition does not, while `--routes-json` and `--features-geo` name the files outright. The **default is unchanged and deliberately so** — scoring against the artwork that actually ships is the reason it read the golden master, and `test/pick_route_colour.test.js` opens with two controls holding it there, both of them falsified in `tools/prove-red.js`. Every run now prints a `sources:` line naming what it opened, because the one thing worse than reading the wrong file is not saying which file. The old workaround — a scratch tree holding the two files copied out of the uncommitted runs, with `--buses` pointed at it — is no longer needed; it is recorded here only because it is what produced the numbers below. **Do it rather than choosing by eye.** Route 20's provisional hue on High Wycombe was `#88CCEE`, picked for a defensible reason — it is what the town-centre boarding sheet already drew the route in, so our two artefacts would agree — and the tool scored it at **dE 6.8 against the river**. Its own answer, `#E69F00`, scored 28.6, which is the number `--stage` reproduces today from the documented path.

**And measure what the tool says its numbers cannot see — BUILT IN since OA-226 (2026-09-03), and a recipe before that.** Its closing advice was that a hue can be numerically distant and still run alongside the line it must be told apart from. The concrete form of "runs alongside" is **shared road edges in `routes_paths.json`**: normalise each edge to an unordered node pair, intersect the candidate route's set with every other drawn route's, and report the overlap as a percentage of the candidate's own line. On High Wycombe route 20 that gave 36 at 49%, 850 at 33%, and 34, 33 and 32/32A at 12% each, with seven drawn routes it never touches at all — so the colours to be far from are five, not twenty-two. `#E69F00`'s worst against that weighted set is the same 28.6, which is what made it safe to take rather than merely top of a list. **The ranking is still the conservative one** — worst case against every colour on the sheet plus every chromatic linear feature — because that is the answer that cannot be wrong for a reason the measurement cannot see; the adjacency figure rides beside it as a second column, and where the two orderings disagree the tool prints a line saying which candidate each one prefers. Making it a `--flag` was considered and rejected: a default-off lever nobody turns on is this project's own recorded failure shape, and a measurement that only argues when it disagrees costs a line of output and is read. A route with no geometry on disk prints `adjacency none on disk`, which is deliberately not the same sentence as "it touches nothing" — the difference between *measured, and the answer is zero* and *never measured* is exactly what kept `gen_external_busway.js` invisible for a day.
...
**And the second implementation got it wrong in the other direction too, for a month — OA-304, fixed 2026-09-12.** The paragraph above closed the March case by teaching the tool the legacy river fallback, and closed it *for rivers only*: the pool was `type === 'river' || type === 'canal'` and nothing else, so on a town that draws a **road** or a **railway** the tool could not see ink it was 2.8 mm wide and in front of it. Asked for Wisbech route 68 on 2026-09-11 it answered `#E69F00` at the top of its ranking, against an A47 the engine strokes `#e6a532` — the same colour to a reader — and its own summary line said, accurately, that it had measured against *1 drawn watercourse*. **The failure is silent and one-directional, which is what makes it worth the row**: the ranking takes the LARGEST worst-case separation, so a missing pool member can only make a candidate look better than it is. There is no version of this bug that produces a finding you would notice. The pool is now every drawn feature whose stroke has chroma ≥ 8 — the engine's own §5.2 threshold, so a `#333333` railway casing is named as *skipped as near-neutral furniture* rather than knocking out half the palette — and the legacy fallback now fires on the **absence of a `features[]` block** rather than on an empty pool, which was a second, quieter instance of the same fault: a town declaring a road and no river got a phantom river in its pool as well as its real road left out of it. The tool's own copy of `FEATURE_STYLES` is joined to `gen_internal.js`'s by a test, because the copy holding two of the five types is how this happened.
...
**SUPERSEDED 2026-09-26 (buses-data OA-437, OA-443): the derivation now IS three.** Two things changed the paragraph above. Since buses-data OA-430 a map is gated against the engine commit that drew it, so moving the template hash raises one rebuild row per map rather than turning the estate red. And three towns built after it — Chatteris, Soham, Whittlesey — stored no `bullets`, placed all six, and so disagreed with the other nine by nothing but where a config had been typed; two of them had already gone to customers. The default is now the first three of the list, plus the not-to-scale bullet under `design.scaleBar:false`. The journey-time bullet went with the two generic ones: every stored town carries times and none ever printed it, and the footer already says *Journey times shown are approximate* wherever a sheet has them. A stored `bullets` still wins, so no map moves until it is rebuilt — and at that rebuild eight of the nine stored lists can be deleted with no change of ink, which is what OA-437's `howToUse.bullets 9→0` counts. **The ninth is not a copy:** The Shelfords stores *in to the Shelfords*, and its `externalHubLabel` would derive *the Shelfords & Stapleford*, so deleting its list changes a word on its sheet.
...
- **A route drawn from its majority pattern names the workings it leaves out** (buses-data OA-452). When S2 wrote `journey_weights.json`, the row gains "· some journeys via Caxton, Eynesbury & Railway Station": the NaPTAN localities of each working of two or more journeys that the drawn line does not already reach, or the working's first stop where it stays inside places the line serves. Too wide for the row, it falls back to "some journeys vary"; a corridor lane gives it a line of its own. `routes.json minorityNote` sets the words per route, or `false` silences one; stderr always names every working with its journey count.
...
**RAISING one needs a note, and `--accept` refuses without it (2026-09-04).** A `REGRESSED` row is a ceiling going UP — the sheet prints fewer labels, or carries more hard defects, or drops more names than the ledger allows — and until now the only thing standing between that and silence was whether the session remembered to write a sentence. Pass `--note "<sheet key>=<text>"`, once per raised sheet, using the key exactly as the report's sheet column prints it (`Huntingdon · schematic`); `--note-file <path>` takes the same grammar one note per line, with blank lines and `#` comments skipped, for prose too long to sit on a command line. **There is no bypass flag** — not `--force`, not `--include-uncommitted` — because a flag that switches a justification off is one that gets typed at six o'clock, and the thing switched off is the only durable record of why the board got worse. A note is only *required* on a raise and is *allowed* anywhere: recording why a ceiling came down is worth as much to a later reader. Two other refusals come with it, both exit 2 and both leaving the file untouched: two notes for the same sheet, and a note whose key matches no sheet being recorded — the second is checked FIRST, because a mistyped key trips both and "this sheet carries no note" sends you hunting for a note you are holding in your hand.

**The same flags, and the same refusal, are on `tools/line-ratchet.js`** (2026-09-04). That ledger records a line-count ceiling per generator rather than a quality ceiling per sheet, and it had the identical hole: `--accept` recorded a 3,286 → 3,400 as readily as a 3,286 → 3,200. It now takes `--note "<relative path>=<text>"` and `--note-file <path>`, refuses a file that is over its ceiling with no note, and appends the paragraph itself. The grammar, the dated append and the order of the two refusals are one implementation — [`assets/ledger_notes.js`](../assets/ledger_notes.js) — so a fix reaches both, and `test/ledger_notes.test.js` asserts both callers use it rather than growing their own back.

**And the note goes in through `accept()`'s own writer, which is the actual bug this fixes.** The per-sheet prose in that ledger had always been typed into the file by hand, so it arrived at whatever indent the editor offered while `accept()` writes **one space**: `099a2b9`, `bd9693e`, `bba5946`, `b82218d` and `bec2cd7` are five ledger commits that each rewrote about 460 of the file's 468 lines to land a single paragraph. The change in every one was a sentence and the diff was the whole file — and "lowering a ceiling is a deliberate, reviewable commit", two paragraphs up, is false the moment the diff cannot be read. The person supplying the prose now never opens the file, so the indent is not theirs to get wrong; the note is **appended** as a dated paragraph after a blank line, never replacing what is there, because the entry underneath is what says which state the one above was measured from. `test/quality_gate.test.js` asserts the one-space indent and `tools/prove-red.js` flips it to two and watches that assertion go red.
...
**AND ITS `DEF` COLUMN IS NOT THE WHOLE SCORE — read the drop count separately (2026-08-30).** `defects` deliberately excludes dropped labels on both sides, so the column this tool prints cannot go up when a change silently loses names. For most of this tool's life the sheets it copied out could not answer that question either: it copies `routes.json` beside every preview sheet, for the reason set out at length in its own header, and did not copy the `unplaced*.json` sidecars, so every preview sheet reported `unplacedLabels: 0` and every HARD figure computed off that tree was missing the whole drop count. Measured on Wisbech internal the day it was found: shipped 7 dropped and hard 8, preview 0 and 0. The sidecars are copied now, and the standing rule is unchanged — **score a round on labels, HARD and drop together**, because a placer rewarded for what is ON the page will always take the easy win. The placer round of 2026-08-30 printed `TOTAL defects: 250 -> 241` for a change that in truth added 4 HARD and 3 drops.
...
`quality_metrics.js` counts two things no measure could express before 2026-08-28: `labelsOverBadge`, a map label printed over a route badge, and `badgeOverBadge`, a badge printed on another badge. A third, `lozengeOverlap`, followed on the same day: two terminus lozenges printed on each other, which nothing had ever measured — OA-060 had been quoting one hand-measured overlap for four days and saying outright that the real population was unknown. It was seven.

**`labelsOverBadge` IS TWO POPULATIONS, and was measured as one until 2026-08-29 (OA-148).** The estate total is **44**, of which **31 are `to X` frame-exit continuation captions** and **13 are placer-attributable**. The measure's own comment had asserted that a caption *"is placed clear of the badge box it belongs to by `termBadge()`'s own `reserve()`"* — asserted, never measured, and false, which is also why nobody re-derived the figure for four weeks. `exitCaptionOverBadge` and `labelsOverBadgeNet` now split them, the same correction `exitTailOverInk` already applies to `pt/ink` (*a point label over its OWN continuation is the design, not a defect*) and using the same `^to\s` name test. The limitation is stated in the source rather than left to be discovered: **it tells a caption from a place name, and it does not tell a caption on its own badge from one on a neighbour's** — Ely Co-op has two captions on one badge, which cannot both be their own. `labelsOverBadge` itself is untouched so the frozen scorecard stays comparable; **the number to drive to zero before the fold-in is the 13.**

**Two of the three are now SCORED and one is not, and the rule that decides which is the same rule.** `badgeOverBadge` and `lozengeOverlap` fold into `hard` as of 2026-08-28, because OA-060 and OA-147 had emptied them first — both start green, the ratchet reports 0 REGRESSED with the fold-in applied, and every future occurrence now fails on the sheet that causes it. `labelsOverBadge` stays in `warns` at 47, because scoring it today would redden every affected sheet at once and a check that is red on the day it lands gets muted within the week. **The gate is the population, never anyone's sense that it is time** — and there is a test plus a mutation that fail the day somebody folds in the third one early, because "tidy up the last one while we are here" would read as housekeeping in review. Both are `null` rather than `0` on a sheet whose `routes.json` will not parse, because the route palette is the whole discriminator between a badge and a stop tick and “could not tell” must not read as “clean”.
...
**Test a badge as a ROUNDED RECTANGLE — not as a circle, and not as a box either.** Take the tolerance from `quality_metrics.js`'s `T.badgeOverlapMm` (0.6 mm) so the generator and the metric cannot disagree about what an overlap is, and use the same function on both sides. Two earlier rules each had a turn and each was exact for the shape its author had in front of them. A RADIAL test is wrong in both directions: under `design.badgeFit` a wide key is a stadium far wider than it is tall, so `hypot < max(rx, ry)` reports tidy neighbours as overprints; and two 2.6 mm discs at dx=5.0, dy=5.0 are 7.07 mm apart yet overlap on both axes, so a radial threshold misses them. A BOX test on both axes is wrong in its own direction, which took another week to find: **two 3.4 mm discs whose centres are 7.22 mm apart have 0.42 mm of clear paper between them and overlapping bounding boxes**, and that is the shape a badge ROW makes on any diagonal spoke — so it fires on the commonest arrangement on the sheet, not on a corner case. Measured 2026-08-28, **17 of the 30 overprints the board then reported were that**, and the honest figure was 13.

The general form both rules approximate is one line. Every mark here is a rounded rectangle — a disc has no straight core, a stadium a horizontal one, a `badgeStack` a vertical one — so the true separation is the gap between the two CORE rectangles less the two corner radii:

```js
const gap = (a, b) => {
  const dx = Math.max(0, Math.abs(a.x - b.x) - ((a.w - a.r) + (b.w - b.r)));
  const dy = Math.max(0, Math.abs(a.y - b.y) - ((a.h - a.r) + (b.h - b.r)));
  return Math.hypot(dx, dy) - (a.r + b.r);        // negative means they overlap
};
```

`w`/`h` are half-extents and `r` is the badge radius the mark was drawn at. It collapses to the radial rule for two discs and to the box rule for two stadiums on one line, which is why both earlier versions looked right. **This is not only a reporting concern**: the placers use the same test, so a conservative rule moves ink to dodge collisions that do not exist — on Godmanchester Ermine Street it walked a badge off a good spot and took the label "Godley Green" with it. And a register of bare CENTRES cannot answer the question at all, whatever the rule.
...
## The numbered place index (`design.placeIndex`, 2026-08-30)

**What it is for.** 288 labels across the 52 committed sheets could not be placed, and every one of them still drew its ICON: an anonymous supermarket trolley, an anonymous mortarboard, a park with no name. The reader could not tell what it was and could not tell that anything was missing either, which is why the whole class was invisible until `unplaced.json` was invented to see it. The index offers each of them a NUMBER at the same point and prints the names under the Key as *Numbered on the map*. The arithmetic is the whole argument: *Cambridge Fish Preservation and Angling Society Ltd. Managed Fishery* is 84 mm of type and `17` is 2.6.

**It is not a second placer.** `labeller.js`'s `indexPass()` re-runs the same solver over the labels that failed, with the text replaced by an ordinal — same candidates, same costs, same ink grid, same hard grid, same leader rules, same boxes already taken. Each accepted marker joins `placedBoxes`, so number 2 avoids number 1 exactly as any two labels avoid each other. The only thing it asks for that a name does not is a closer nominal gap (`it.gap`, 1.7 mm rather than 2.6): a marker is a tick, not a name, and holding it out at name distance on a saturated sheet is the difference between placing it and dropping it.

**Four decisions, each of which can be got wrong on its own.**

| Decision | What it is, and what the other answer costs |
|---|---|
| It does **not** force | A point with no room for two digits keeps its silence. A number stamped on a route line is one a reader can neither read nor look up, so forcing would turn a visible failure into an invisible one — and would drive the ratcheted drop count to zero for nothing |
| Placed on the **widest** ordinal | Every marker is placed against the widest number the pass could issue, so the box does not depend on the digit count. Numbering first forces a choice between a gap in the sequence and a box narrower than the glyphs |
| Numbered **alphabetically**, after placement | A reader arrives from two directions — with a number wanting a name, and with a name wanting to know whether the map shows it. An alphabetical list ordered by its own numbers answers both; placement order answers only the first |
| **Fewest** columns that hold the rows | Every column past the first takes width from every entry. Choosing the columns before the row count is known shortened *St Mary and All Saints CofE Primary* to fit a half-width column Beaconsfield did not need |

**Two files, and the split is deliberate.** `unplaced.json` keeps its old meaning and holds the RESIDUE — what the sheet still does not say; `indexed.json` records what was numbered. Folding a numbered label into "placed" would take the ratcheted DROP figure from 288 to 202 the moment the code landed, which is a target bought with a definition change rather than an improvement.

**Where it cannot reach, it says so, and which constraint bit.** *The index block is FULL* and *the map has no room beside those symbols even for two digits, and the index block still has N free rows* have opposite remedies, and `rollout.js` captures both into `build-warnings.txt`. High Wycombe prints the second on all three sheets, every build: 17 of 165 numbered there against 68 of 98 everywhere else. That is not a tuning problem and no placer change reaches it — it is the sheet saying it is the wrong size.

**UPDATED 2026-09-01 (OA-187, OA-213) — two of the paragraphs above are now history, and both are left in place because the reasoning is still what a reader needs.**

**`max` is the block's CAPACITY, not a budget of attempts.** `indexPass` used to slice its candidate list to `max` before trying to place anything, so every candidate the map had no room for spent one of the caller's ROWS and produced none — a budget counted in attempts against a block measured in successes, which agree only on a sheet where nothing fails. High Wycombe was the only town on the board dense enough for that to bite, and it bit hard: the paragraph above quoting *17 of 165 numbered* is from that era. The walk now stops when the block is full, under a separate and deliberately generous attempt ceiling (`min(want.length, max(cap*4, cap+40))`) so a 260-name sheet cannot run `_best` 260 times. **The residue message is now TRUE everywhere** — it always said the map had no room beside those symbols, and on High Wycombe that was false for 43 of them, because they had never been tried. Twenty-two more names carry a number, and no other sheet in the estate moved a byte.

**The marker is 2.4 mm, not 2.3.** It was below `quality_metrics.js`'s own `T.minTextMm = 2.4` print-legibility floor, so **every index marker on the estate was a HARD defect** — 82 of them, 27% of the whole board, in a bare constant restated in two files with nothing to be wrong against. Measured across all 39 rebuildable sheets before it was taken: HARD 287 → 183, and the entire cost is ONE additional point-label-over-ink. **No sheet drops a single extra label**, which is the trade the row expected to pay and did not. The floor itself is untouched — the artwork moved, which is the difference between fixing this and redefining it. `labeller.test.js` now reads `T.minTextMm` from `quality_metrics.js` rather than restating the number, so the two cannot drift apart again.

**And it is why the two had to ship together.** OA-187 alone leaves the HARD count at 287: every name it rescues leaves the `unplacedLabels` term and arrives in the tiny-text term in the same build. A sheet gets better and the board records nothing.

**And the block obeys what is already in the panel column.** Its capacity is walked a row at a time and stops at the first row that lands on real ink, asking `whatBlocksInk` rather than `overlaps`, because the services-panel box covers the column this block is deliberately drawn in. High Wycombe authors four `mapNotes` at `x=200` on purpose; dividing the distance to the footer plate would have printed the index straight through them.
