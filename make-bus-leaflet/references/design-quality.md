...
**And the report says how many sheets could not supply the number.** 31 of the 52 cannot count dropped labels at all — every external sheet returns `null` rather than a figure — so the "108 dropped labels" that reads like a board-wide total is a total over 21 sheets. Summing an unknown as a zero would let a board that measures **less** report itself closer to target, which is the same trap the label floor exists to close. The line prints `31 of 52 sheets could not count it` beside the number instead. That gap is an open backlog item, not a solved problem: the drop target cannot honestly be claimed as met until the population is either fixed or stated.
