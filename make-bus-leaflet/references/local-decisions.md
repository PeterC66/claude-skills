...
**What exists today is Phase 0, recording, for six of the seven question types — and Phases 1 and 2 for landmarks alone** (type 5, built as OA-212 and OA-233 on 2026-09-01 and 2026-09-05: the portal's landmark chooser at `/app/maps/:id/landmarks`, `GET /api/maps/:id/poi-tiers`, and `poi_tiers_sync.js` in this folder, which writes the answer into a new S3 run). A build writes its local questions to a file. **Two things read it.** `bus-work`'s worklist raises a row for each map's `asked` decisions and another for its never-asked ones, towns and places alike. And **since 2026-09-26 the portal's `deliver-map.mjs` step 0c refuses to deliver a map while any `blocking` decision is unanswered** (buses-data OA-083, community-bus-maps#393): only `answered` and `dont-know` clear it, with dated per-decision deferrals in the portal's `scripts/local-decision-waivers.json`. Peter ruled the same day that the six non-landmark types go to the customer **by letter**, not into the editor, so no portal panel reads the file and an answer reaches it by hand.
...
```json
{
  ...
  "kind": "place",
  "updated": "2026-08-25",
  "_status": "Recorded, and read by two things: bus-work's worklist.mjs raises a row for each map's `asked` decisions and another for its never-asked ones, in Areas/<town>/ and in its Places/; and the portal's deliver-map.mjs step 0c refuses a delivery while a `blocking` decision is unanswered. An answer reaches a sheet by hand, and `appliedIn` names the S3 run where that happened.",
  "decisions": [
    {
      "id": "c2-stub-name",
      "type": "naming",
...
- **`severity`** — `advisory` or `blocking`. `blocking` is reserved for the case where printing the wrong answer misdirects a passenger: type 4 always, type 3 when the wrong answer means a service is printed as running. **It is enforced at delivery since 2026-09-26**: `deliver-map.mjs` step 0c refuses a map with an unanswered `blocking` decision, so mark one `blocking` only when that refusal is what you mean.
...
Phase 1 puts these rows in the portal for the customer's editor to answer; Phase 2 pulls the answers back down into this same file's `answer` fields and seeds S3 from them on every subsequent build; Phase 3 makes an outstanding `blocking` decision stop a publish, with expiring waivers in the shape `s6-waivers.json` already uses. **Phase 3 is BUILT** (2026-09-26, as `deliver-map.mjs` step 0c and `scripts/local-decision-waivers.json` in the portal), and Phases 1 and 2 are built for landmarks only: Peter ruled on 2026-09-26 that the other six types go by letter rather than into the editor. Full reasoning and phasing: `…\Buses\Development Docs\local-decisions-to-the-editor-plan_2026-08-25.md`.
