...
2. **Full both-direction chains (bustimes).** Get the service slugs from the locality page:
   ...
   `node "%SK%\pull_routes_full.js" route_slugs.json . --dates 2026-07-23,2026-07-25,2026-07-26`  → `routes_full_atco.json` + `_all_atco.json` (+ `svc_<slug>_<date>.html`).
...
4. **Derive the in-town display subset.** Write **`intown_cfg.json`** then `node "%SK%\derive_intown.js" routes_full_atco.json atco2ll.json intown_cfg.json routes_intown_atco.json`, and `cp routes_intown_atco.json routes_atco.json`.
   `intown_cfg.json = { "prefix":"0500HSTIV", "extraCore":["0500HHOLY010"], "buf":1, "circular":["300"], "anchor":"0500HSTIV002", "maxEdgeKm":3.5, "coreMaxKm":2.5 (only where the ATCO block is over-broad) }`:
