...
6. `stage.js commit S4 "$S4" --outputs internal.svg,external.svg[,internal-schematic.svg] --based-on "S2=$(basename $S2dir);S3=$(basename $S3dir)"`.

## Stage 5 — Render (same version as S4)
1. `S5=stage.js new S5` (inherits S4's version); `cd "$S5"`.
2. `stage.js pull S4 .` → `node "%SK%\render.js" internal.svg internal.jpg`; `node "%SK%\render.js" external.svg external.jpg`; plus `internal-schematic.svg → .jpg` when present. `sharp` is bundled in the skill, so `render.js` runs from any folder.
...
4. `stage.js commit S5 "$S5" --outputs internal.jpg,external.jpg[,internal-schematic.jpg]`.
