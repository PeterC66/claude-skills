...
Do make the *orchestration* cheaper. Two rules, both learned by getting them wrong in one session:
...
2. **Have the agent WRITE `redteam.json` itself — do not let the JSON travel back through the orchestrator's context.** Add to the prompt: *"Write the JSON to `<absolute path>/redteam.json` and reply with one line: the town name and the number of services found."* Returning a 5–10k-token JSON blob and then re-typing it into a file pays for the same content twice, once reading and once writing. Across seven towns that alone was roughly **100k tokens of pure duplication**.

A seven-town clear-the-board exercise is two or three sessions' work. Plan it that way rather than discovering it.

## Known limitations of the checks themselves (added 2026-08-26)

These produce findings that look like defects and are not. All were settled by opening the rendered sheet — **when a direction or terminus finding surprises you, look at `_latest/internal.jpg` before believing it.**

- **S-5 direction only asserts anything when a route has at least TWO non-core buffer stops.** The code picks the edge stop from `seq.filter(a => !isCore(a))`, and its own comment explains at length why the farthest *drawn* stop is the wrong choice. Then `const cands = edgeCands.length ? edgeCands : seq` falls straight back to that rejected behaviour whenever a route has **no** buffer stop, comparing an in-town stop's bearing against the terminus — which asserts nothing. With exactly **one** buffer stop the selector has no choice and returns it regardless of direction. On 2026-08-26 all four direction HARDs across seven towns (Huntingdon T1, March 32, Ramsey 32, Ramsey X31) came from routes with fewer than two buffer stops; the only two findings from routes with a genuine choice (St Ives 301, 5A) were SOFT. Ramsey's 32 and X31 were flagged "drawn the wrong way" on the strength of a single Bury stop 207° south, on a sheet that correctly draws both leaving north-east under "to March" and "to Peterborough". **Check the buffer-stop count before acting on a direction HARD.**
- **Route-key normalisation double-counts a route whenever the red team appends a brand name.** The agent writes what the operator brands, so `ZIP2 (Ely Zipper 2)`, `102 (Flightline)` and `46 (Lynx)` normalise to keys our data does not hold — and the same route then appears **twice**, once as `not-confirmed <ours>` and once as `missing-service <theirs>`. Seen on March, Beaconsfield and Wisbech in a single run. The Wisbech case is the dangerous one: `missing-service 46(LYNX)` reads as a gap when our data already carries **both** route 46s correctly (Stagecoach East to March, Lynx to King's Lynn). Reconcile by operator and termini before treating any `missing-service` as real.
- **A truncated `routes_full_atco` chain reads as a terminus HARD.** Wisbech's `excel` holds only the 15 local stops, both ends inside Wisbech/Wisbech St Mary, while its declared termini are Peterborough and Norwich — so "neither declared terminus appears at the ends of its full chain" is true and means nothing about the sheet, whose external config correctly draws two spokes. Check `routes.json`'s `external[]` before believing a terminus HARD on a long-distance route.
- **A large SOFT count is not a large defect count.** High Wycombe returned 97 soft, almost all of it the red team enumerating variants (1S, 32/32A, 36/36S, 130D, 130M, 300M) and school routes (604, 643, 644, 654, RGS01/02A/02B/02C) at a finer granularity than the map deliberately models. Read the category breakdown before quoting a number at anyone.

## An uncurated S1 makes S6 unreadable (added 2026-08-26 — Ramsey)

If a town's `verified-services.json` came from `draft_town.py` and was never replaced by a real S1 pass, its `termini` hold **stop names** from GTFS headsigns where every other town holds **settlement names**. Every terminus check then fails by construction — Ramsey returned **12 terminus HARDs on that basis alone**, seven structural and five from the red-team diff, and **the five are not independent corroboration, because both checks read the same field.** The sheet itself was fine throughout: its Services panel is built from `routes.json`, which was correct.

**Before running S6 on any town, check `routes.json` for a `_bootstrap` field and the town folder for `DRAFT-REVIEW.md`.** If either is present the S1 pass is owed first; S6 will otherwise generate a large, confident, unactionable HARD count that buries the findings that matter. Ramsey's own `DRAFT-REVIEW.md` had predicted every finding the run produced — including the missing community services and the untuned "to X" arrows — and had sat unread since 2026-08-04 while the map was live.
