...
3. **Ask whether this town already owns a red-team answer.** Run `node "%SK%\redteam_source.js"` from `$S6`. It reads every `redteam.json` this build has ever bought and decides: **REUSE** (it copies the answer into `$S6` and you skip step 4 entirely) or **BUY** (spawn the agent). **It names the resolved `--build` path on every run** (2026-08-29, OA-141) and **refuses (exit 2) when the folder is ambiguous** — when its default `../..` hop and the nearest enclosing `manifest.json` above `--into` name different maps. Run from the documented cwd the two agree and nothing changes; run from a PLACE's own root they do not, and it used to answer about the parent town in silence. `--foreign-build` lifts the refusal on purpose — see *Running S6 on a PLACE*, item 4. It reuses only when our S1/S2 inputs have not moved since the answer was derived AND the answer is inside a 60-day window; it exits 0 for reuse, 10 for buy, and 10 is the normal outcome for a town whose data has just been re-pulled, not an error. The reasoning behind both conditions — in particular why blindness does not decay with age but accuracy about the world does — is written at the head of the script. On the estate as it stood on 2026-08-27 this reused 9 of the 18 builds that hold an answer. **If you reuse, say so in the S6 commit note** so the run records whose research it rests on.
...
6. **READ A PLACE'S SOFT COUNT AS A FACT ABOUT THIS CHECKER, NOT ABOUT THE MAP.** The seven runs of 2026-08-29 produced soft counts from 11 to 113, and **283 of the 353 carry no information**. Three sources, all of them the *four noise sources* shape described above, and all three are recorded as OA-156 in the `buses-data` backlog with the measurement behind them. Until they are fixed, subtract them by eye before quoting a number to anyone:
   - **176 terminus findings compared against nothing.** Each carries `evidence.chainEndTokens: []` — our drawn chain offered no NaPTAN locality code at either end, so nothing was compared. The red-team variant's wording is the harmful part: *"NEITHER red-team terminus … is a locality at the ends of our drawn chain"* states a disagreement where the truth is that no comparison happened. Worst on a **boarding-only** place — High Wycombe Town Centre and High Wycombe High Street both carry `routes[]` of length zero and no `destinations[]`, deliberately — but present on every place tried.
   - **78 `missing-service` leads that are the borrowed answer's own breadth.** A town answer is a superset of a place's services, so every town route that does not call at the place reads as an inclusion candidate. High Wycombe Aldi draws 12 against a borrowed answer of 44. This is the OA-141 scope gap arriving in the direction OA-141 did not anticipate — it reasoned about the town answer being legitimately *silent*, and what dominates is the reverse. **Do not filter these blind**: St Neots Co-op's W9/W10 leads are real and are already OA-050.
   - **29 of 64 `days` findings that differ only in wording** — ours *"Sat"* against the red team's *"Sat only"*. Our string is a case-insensitive prefix of theirs in all 29. A further 15 are our own value reading `"?"`, which is a genuine gap worth keeping separate.

7. **The direction check reports its own coverage now, and for a place that coverage is always zero** (2026-08-29, OA-048). `summary.directionCoverage` carries `checked / unavailable / skipped / displayed / pct` plus `accountsForAll`, and the console prints the fraction beside the verdict. A place always reads `0%` and its `direction-unavailable` finding carries `allBlind: true` — that is by construction, not a fault, and Peter decided on 2026-08-29 that places stay out rather than get a check of their own shape. Worth knowing what the same number says about TOWNS: measured across all eight on 2026-08-29 the check ran on **26 of 95** drawn routes, and on High Wycombe and March on none at all.
...
These four produced findings that looked like defects and were not. Until 2026-08-27 they were documented here as things a reader had to remember to discount; they are now handled in `verify_report.js`, and each fix is paired with a proof that the same check still fires on a genuine fault of its kind — `npm run test:prove-s6`, 22 assertions, which goes red on 12 of them against the pre-fix engine. The old advice is kept below as the record of what each fix is for. **The general rule still stands: when a direction or terminus finding surprises you, look at `_latest/internal.jpg` before believing it.**
