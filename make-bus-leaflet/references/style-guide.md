...
- **Water is `#9ec9e8`**, and **no route may wear a colour close to it in both Lab distance and hue.** The engine warns; `pick_route_colour.js` proposes a replacement. Score a candidate against **every other colour on the sheet plus every linear feature it draws in a chromatic colour** — the roads and railways as well as the rivers, since OA-304; it was water-only until 2026-09-12 and offered Wisbech the A47's own amber — and take the largest *worst-case* separation — a hue can be numerically far from the river and land next to the route it now has to be told apart from.
