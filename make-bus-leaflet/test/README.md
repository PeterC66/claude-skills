...
**`place_assets_load.test.js` asks the same question one skill over, and the answer was that nothing had ever asked it** (added 2026-09-12, buses-data OA-001, filed onward as OA-323). The test above derives its population from `engine_version.js`, which is this page's own recommendation — and that derivation reaches **2 of the place skill's 14 assets**. Eight of the fourteen are named in **no test and no harness anywhere in the estate**, measured by walking all 126 files under `test/` and `tools/` with the two place generators as the control that the instrument works at all. That is not the hash lists being wrong: they exist to name what DRAWS a sheet, and a stage tool does not. It is that nothing else was asking, and **a derivation source that cannot see a whole skill is indistinguishable from coverage, exactly as a typed list is** — so *derive, never type* does not by itself close this.

**The reason the eight are dark is structural.** OA-224 Tier 4.1 put every generator's body behind `require.main === module` so the test above could require one; that round reached this folder's two GENERATORS and stopped. The other eight `.js` act at load, so `require()` would run them — confirmed by giving `derive_termini.js` the guard in a scratch copy and watching it die on `ENOENT routes.json`, rather than by reading the plan. **The cheapest question in the estate is therefore unavailable to six sevenths of the place skill**, which is the state `gen_external_busway.js` was in for a day with every gate green.

Population from the DIRECTORY listing, not from any list; classification by BEHAVIOUR, so a file that gains its guard changes group by itself and the entry excusing it goes red. Four properties: every `.js` **parses**; anything requireable is required with the cwd set to an empty directory and must draw nothing; a file that is neither guarded nor excused in `NOT_REQUIRE_TESTABLE` fails, so a new script cannot join a silent majority; and that map — the inventory of what is still dark, one reason each — **retires its own entries in both directions**. **What it does NOT claim is the load check itself**: a `ReferenceError` at module scope parses, so `node --check` would have called the busway file healthy. Parse is merely the strongest question available to a script that cannot be required, and selling it as more would be a check reporting on a predicate it never evaluated. All 14 pass it today, so the gate is green on the day it lands rather than red — the rule that a gate red on day one is one somebody mutes in its first week.

**Falsified four ways with the control run FIRST, and then the control that matters more.** A new unguarded asset, an exempted file that gains the guard, an exempted file that is deleted, and a file that will not parse, each watched go red against a scratch copy reached through `PLACE_SKILL_ASSETS` — with an unmutated copy green first, because otherwise every mutation "passes" while the test is quietly reading the real folder. Then: *would the committed suite already have caught these?* With an unparseable `derive_walkshed.js` in the folder, the whole suite reports **exactly one** failing test and it is the new one. 902 tests see it; 896 did not. **The place half skips loudly** here too, for the reason given above.
...
`tools/prove-red-portal-drift.js` is OA-200's falsification, and it needs no other repository: it BUILDS a portal — a git repository in the OS temp dir holding a manifest, two vendored copies of real skill files, and a `refs/remotes/origin/main` written with `git update-ref`, which is all `git rev-parse` and `git show` need in order to answer exactly as they do against `C:\Claude\community-bus-maps`. Nothing under the real portal is read, written or checked out. Seven cases cover the three trees the verdict sits between: the ref in sync; the ref stale; a re-vendor uncommitted in the working tree, which must read `PENDING` rather than green; an unlisted `.js` on the ref; an unlisted `.js` on a branch only, which must NOT be named; a portal that is not a git repository, whose disk fallback must say so; and the row itself — a stale CHECKOUT while `origin/main` is clean, which must stay green and must name the branch it declined to read.

**It falsifies itself, and that is the half worth copying.** Seven green rows are also what a harness asserting nothing prints, and the two cases about *which tree was read* are green under the fix and were red before it — a distinction no exit code carries. So the two are re-run against a scratch copy of `status.js` with the one line of OA-200 removed (`const ref = null`), and each must go red **naming the row the old reading produced**: `DRIFTED` on `qr.js`, `UNLISTED` on `extra.js`. The first cut scored both as "goes red" while they were in fact reading `MISSING` — a copy of `assets/` running from the temp dir resolves every manifest `source` to a path that does not exist, so the reds had nothing to do with trees. The mutation now rewrites `SKILL_ROOT` to a literal as well. A harness that accepts any red is not a harness.
...
**Twenty-four mutations — read the count off the run, not off this line — and the control is the half that earns the file.** `tools/prove-red-python.py` copies `assets/` to a scratch directory, applies one edit per case, runs one test file against the mutated copy and requires it to fail; every anchor must match exactly once, asserted before the edit is made. Case 0 runs the whole suite against an UNMUTATED copy and requires it green — without it, a scratch copy that never received the engine would report every mutation as caught, each run failing for a reason that has nothing to do with the mutation. **Both arms were watched fail before this was committed**: a mutation the suite cannot see (a reworded comment) is reported SURVIVED with exit 1, and a deliberately broken control stops the run before grading anything.
