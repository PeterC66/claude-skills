...
That is `node --test`, which finds every `test/*.test.js` file from the package root. **246 tests** as at 2026-08-27, about a second, no network, no data tree, no `Areas/` folder needed. That figure moved four times in a fortnight — 114, 170, 227, 246 — so read it off the run rather than off this line.
