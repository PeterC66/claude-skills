...
Neither pre-stage can be required, so the rest of that suite is SOURCE pins, and one of them defends a decision rather than an algorithm: `LEGACY_FRAME` must still say `FOOTER_SAFE: false`, because flipping it is the adoption — a drawing change with a version bump on up to 13 sheets — and it has to arrive as a deliberate edit to the test, never as a quiet edit to the generator. `prove-red.js` flips it in each pre-stage in turn and expects the pin to object. `internal_roads_config.test.js` makes the same move for the one reading of `internalRoads`: the estate cannot certify an ABSENT key, because every schematic town writes the block, so the suite is the only place that rule is held.
