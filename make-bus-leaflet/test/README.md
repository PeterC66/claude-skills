...
**Fourteen mutations, and the control is the half that earns the file.** `tools/prove-red-python.py` copies `assets/` to a scratch directory, applies one edit per case, runs one test file against the mutated copy and requires it to fail; every anchor must match exactly once, asserted before the edit is made. Case 0 runs the whole suite against an UNMUTATED copy and requires it green — without it, a scratch copy that never received the engine would report every mutation as caught, each run failing for a reason that has nothing to do with the mutation. **Both arms were watched fail before this was committed**: a mutation the suite cannot see (a reworded comment) is reported SURVIVED with exit 1, and a deliberately broken control stops the run before grading anything.
