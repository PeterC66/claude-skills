...
```
python3 "%TSK%\gtfs_duration.py" <ATCO_PREFIX...> --fill-place routes.json
python3 "%PSK%\derive_stops.py" routes.json --dir .
```
...
- `derive_stops.py` also writes `stopLocalities` beside every `stops` array lacking one — the same length, each entry the NaPTAN `LocalityName` of that stop or `null` — so the portal's `/maps` search can match a stop by the settlement it is in rather than by the shape of its name (buses-data OA-311). A stop it picked is joined on its ATCO code and is exact; a hand-kept `stops` list is resolved by name against the destination's own routes' chains and left `null` wherever those disagree (two *Bus Station*s, two *Church Street*s) or the name is not a stop. It reads `_gtfs/naptan.sqlite`, found by walking up from `--dir` unless `--naptan` names it, and without it writes no localities and says so on stderr. No sheet draws the field, so it moves no ink.
