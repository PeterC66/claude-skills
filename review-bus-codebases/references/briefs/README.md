...
**Do not add context to a brief.** On 2026-09-03 the consolidating session prepended a summary of what the previous round had landed, and one item in that summary named a file that did not exist; the reviewer had to spend a finding correcting the brief. The reviewer already receives the previous findings document and is told to read the plan's *Where this stands* table; that is the whole of what it needs to know about the last round, and it is verified there rather than restated here. The only things filled in are the date and the two paths.
