...
**And because the objection is ownership, `--checkout` is how you opt IN the worktree you are standing in** (12 September 2026). The exclusion above is right and stays: nothing goes LOOKING for worktrees, because that is what let two sessions stamp each other's files. But a session working in a worktree still has to stamp its own documents before committing them, and until this flag existed the only way was to copy `stamp-policy.json`, rewrite the root's `path` by hand and pass `--policy` — which was done, worked, and is exactly the kind of workaround that becomes folklore. `--checkout [DIR]` retargets **one** root at that directory and walks **that one checkout and no other**, so the property the incident bought is kept: you can still only ever stamp the tree you are in. It resolves the root by asking git — a linked worktree's `--git-common-dir` is the main checkout's `.git`, whose parent is what a root names — so it works whatever the directory is called, and `cbm-r2-sample` is not a name any list would have guessed. From inside the worktree, with no placeholders:

```bash
python "C:/Users/Peter/.claude/skills/stamp-docs/scripts/docstamp.py" --all --checkout .
```

**It REFUSES a directory it cannot place, and that is the opposite of the auditor's default on purpose.** `check_committed_stamps.py` audits an unmatched repository with no exclusions and says so, because a silent filter is the worse failure when you are only reading. This script writes, so stamping an unknown tree with no exclusions would edit the very files the policy exists to leave alone — it stops instead and names the fix, `--root NAME`. Both directions are in `prove_policy.py`, and the refusal was watched go red by deleting it from `resolve_checkout()` and re-running: `MISS an unmatched checkout is REFUSED — the control (it was accepted)`. **Its first fixture was a fake**, a `.git` file written by hand with the right `gitdir:` line, and `git rev-parse` refused it — a linked worktree is also an entry under the main checkout's `.git/worktrees/`, which writing the leaf file does not create. The harness now builds a real repository and a real `git worktree add`.
