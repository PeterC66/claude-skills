---
name: stamp-docs
description: Keep the bus-system documents (Markdown and PowerPoint) correctly stamped and correctly formatted — a visible version number and last-updated date that refresh themselves, and Markdown paragraphs kept as whole unwrapped lines. Stamping runs by itself at commit time, from each repository's pre-commit hook; invoke this skill to check the current state, backfill newly added documents, force a major-version bump after a rewrite, change which documents are in scope, or unwrap hard-wrapped Markdown. Use when asked to check/fix document versions or dates, "what version is this doc", "did the stamp update", "add the new folder to stamping", "bump that doc to v2", or to fix/unwrap/reflow hard-wrapped paragraphs in .md files.
---
...
Until that day a Stop hook stamped the working tree after every turn, so a commit made mid-turn carried the new content and the old stamp; the story is in [README.md](README.md#what-keeps-it-up-to-date). The hook and its `--auto` mode are both gone — `--auto` was deleted from the script on 21 September 2026.
...
The stamp carries a short hash of the document's **own content with the stamp removed** — for a deck, all slide text plus every embedded media blob, so a swapped image counts as a change. If the hash is unchanged the file is **not rewritten at all**, so mtimes never churn and a document re-committed unchanged keeps its version.
...
python scripts/docstamp.py --all                # hash scan; stamps whatever changed
...
**The cost was never the wasted work — it is that a stamp is an EDIT.** Two sessions could not both run `--all`: on 3 September 2026 each bumped stamps on documents inside the other’s worktree, neither having touched the other’s files, and the standing rule then was *re-stamp a document you edited BEFORE you commit it*, so a stamp arriving from elsewhere was exactly what got swept into a commit describing something else.
...
**A worktree is placed through git, never by its name.** A linked worktree's `--git-common-dir` is the main checkout's `.git`, whose parent is what a root names, so `docstamp.py --staged` in the pre-commit hook stamps a worktree's commit with its root's exclusions whatever the directory is called — and since 21 September 2026 the audit places it the same way, through the same function (`root_cfg_for_repo()`). `--checkout` uses that resolution too, and **refuses** a directory it cannot place, because it writes; the audit, which only reads, audits an unplaced repository with no exclusions and says so on stderr. Both directions are proved in `prove_policy.py` and `prove_staged.py`, against a real `git worktree add` rather than a hand-written `.git` file, which `git rev-parse` rejects.
...
Read it back rather than assuming: on 2026-08-25 the portal briefly went **3 STALE of 29** in `main` this exact way, and reads 32/32 once the follow-up landed. The checker exits 1 on findings, and **it runs in CI**: the `status` job of `buses-data`'s `.github/workflows/gates.yml` runs it from its checkout of this repository — pinned to the engine commit in `engine.lock.json`, so a change to this script reaches that job when the pin next moves — over `buses-data` and `community-bus-maps`, naming those two roots explicitly. The third policy root, `community-bus-maps-ops`, is deliberately NOT passed — it is never pushed, a runner has no copy of it, and it holds the estate's one genuinely stale stamp, so judging it from CI would make the step red on day one for a reason nobody could fix. Proven both ways on 2026-08-27: exit 0 on the real corpus (Buses 82/82, portal 32/32 by explicit root), exit 1 on a scratch repository holding one deliberately wrong sha.
...
The audit honours the policy's exclusions, so `_archive` and other out-of-scope trees are counted (`N out of policy scope`) rather than judged — **provided it can place the repository at a root**, which inside a linked worktree it could not until 21 September 2026 (see the last paragraph of this section).
...
**And the same bug was one level out, in CI, where the fix above does not reach.** `gates.yml` audits by NAMED path — `check_committed_stamps.py "$GITHUB_WORKSPACE/buses-data" …` — and a named repo was documented as *audited without exclusions*, because the policy config travels with the policy ROOTS and a CI checkout is at a path no root has. So the local hook went quiet and CI went red about the same nine files, on the same push. Policy roots now carry **`checkoutDirNames`**: the names the same repository goes by when it is checked out elsewhere (`buses-data`, `community-bus-maps`). **A repo matching nothing is still audited with no exclusions and SAYS so**, on stderr, from both the HEAD path and `--staged`, because a silent filter is the worse of the two failures. Falsified in a throwaway repo with one stale `_archive/` file and one stale `Documentation/` file: named `buses-data` it reports 1 STALE and 1 out of policy scope, and renamed to anything else it reports 2 STALE and prints the warning.

**And the placement itself was a second copy of the stamper's, which is how a worktree fell through it (21 September 2026).** The audit matched a repository by path and then by `checkoutDirNames`; a linked worktree has neither, so every commit made in one had its staged documents audited with no exclusions and no warning, while the stamper — placing through `--git-common-dir` — correctly left `_archive` alone. A merge in a `community-bus-maps` worktree was refused over `docs/_archive/GO-LIVE.md`. The audit now asks `docstamp.root_cfg_for_repo()`, so the two cannot place a tree differently. `prove_staged.py` case 9 commits a stale-stamped `_archive/x.md` inside a linked worktree named nothing any list holds, with a stale in-scope control beside it; case 10 requires the no-match warning. Both went red against the old placement before the fix.
