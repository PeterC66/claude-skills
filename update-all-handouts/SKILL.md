...
7. **Web-manager feedback (site-issue watch).** Each child skill separately tracks genuine **site problems** noticed while scraping — stale/placeholder links, name mismatches between a page's heading and body, internal-looking editorial notes accidentally published in visible text, inconsistent status wording — as distinct from ordinary month-to-month content changes (see each child's "Site-issue watch" section). Collect what each of the three runs found (including explicit "nothing found" for any that were clean) and, if anything was found across any of them, offer to draft a short feedback email to the user's web manager: grouped by which handout's scrape surfaced it, plain description of the issue with a link/example, and a line confirming any handout whose scrape was clean. This skill only drafts the text for the user to review and send — it does not send anything itself (sending email is a user action per the assistant's operating rules).
