...
4. **Scrape the three list pages**; for each new item fetch its detail page for full text + the image URL. Detail pages routinely carry short notes like "(Free image)" or "(Image generated by AI)" — these are normal editorial/licensing annotations, not a site bug; ignore them (don't copy them into the handout, and don't flag them as a site issue). While scraping, separately note anything that *is* a genuine **site problem** rather than an ordinary listing change — inconsistent status wording vs. the rest of the site, a stale/placeholder link, a name mismatch. See "Site-issue watch" below.
...
## Site-issue watch
This handout is an extract of the live site, but scraping list + detail pages also surfaces genuine site problems that have nothing to do with this month's listings (status wording that doesn't match the site's usual pattern, a stale link, a name mismatch). **Ordinary editorial/licensing notes such as "(Free image)" or "(Image generated by AI)" on detail pages are expected and are not a site issue — don't report them.** For anything that *is* a genuine issue, don't fold it into the doc edit or silently ignore it — mention it in the final hand-off to the user (or, when running under `update-all-handouts`, hand it to the orchestrator so it ends up in one combined note) so it can be passed on to the web manager. If nothing was found, say so explicitly.
